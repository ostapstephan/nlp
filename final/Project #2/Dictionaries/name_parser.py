...
#Run with: python name_parser.py path_to_names_folder old_dictionary.txt new_dictionary.txt
...
for file in os.listdir(args.texts_path):
    ...
    with open(os.path.join(path, file), 'r', encoding='iso-8859-1') as f:
        for line in f:
            word = line.split()[0]
            if word.lower() in frequency_dictionary:
                del frequency_dictionary[word.lower()]
            if word.title() in frequency_dictionary:
                del frequency_dictionary[word.title()]

            frequency_dictionary[word.title()] = 1
